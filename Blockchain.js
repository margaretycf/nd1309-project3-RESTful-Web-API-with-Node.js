...
class Blockchain{
    ...
    return new Promise(function(resolve, reject) {
      ...
      self.getBlockHeight().then((height) => {
        ...
        if (newBlock.height > 0) {
          self.getBlock(newBlock.height - 1).then((pre_block) => {
  ...
  getBlock(blockHeight){
    ...
    return new Promise(function (resolve, reject) {
      db.getLevelDBData(blockHeight).then((blockString) => {
        if (blockString === undefined) {
          reject('Not found');
        }
        resolve(JSON.parse(blockString));
      }).catch((err) => { reject(err) });
    });
  }
  ...
  async validateLink(blockHeight){
    let self = this;
    return new Promise(function (resolve, reject) {
      self.getBlock(blockHeight).then((current_block) => {
        self.getBlock(blockHeight - 1).then((previous_block) => {
